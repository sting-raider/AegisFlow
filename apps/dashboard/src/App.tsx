...
function SystemHealth({ status, connected, flows }: { status?: SystemStatus; connected: boolean; flows: Flow[] }) {
  ...
  return <><div className="metric-grid">
    ...
    <Metric label="Detection queue" value={(status?.queue.lag ?? 0) + (status?.queue.pending ?? 0)} note="lag + pending" />
    ...
    <section className="panel ledger-panel"><header><span>Service readiness</span><small>reported only</small></header><dl><div><dt>Sensors</dt><dd>{status?.sensors ?? "not reported"}</dd></div><div><dt>Suricata</dt><dd>{status?.suricata_status ?? "not reported"}</dd></div><div><dt>Dropped records</dt><dd>{status?.dropped_records ?? "not reported"}</dd></div><div><dt>Worker latency</dt><dd>{status?.worker_latency_ms === undefined ? "not reported" : `${status.worker_latency_ms.toFixed(2)} ms`}</dd></div></dl></section>
