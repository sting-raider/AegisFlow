...
| Open-set calibration | Bundle v3 persists a bounded benign-calibration empirical CDF; runtime reports its right-rank/interpolated percentile separately from normalized score; grouped calibration and test partitions are disjoint | **In progress:** implementation and synthetic evidence complete; public held-family/cross-dataset validation remains tied to exact hybrid evaluation |
| Evidence-backed fusion | Every weight/threshold loads from the bundle; an 82-configuration transparent calibration loop compares baseline and selected rules on disjoint grouped smoke data and records both confusion matrices | **In progress:** configurable comparison machinery is complete; public held-family/cross-dataset evidence is still required before operational preference |
