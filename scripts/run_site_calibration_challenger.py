...
from training.research.hybrid import run_cross_fitted_site_calibration
...
def _markdown(report: dict[str, Any]) -> str:
    lines = [
        ...
        "",
        "| Strategy | Runs | Mean direct detection | Worst direct detection | "
    ...
    lines.extend(["", "## Primary held-family results", ""])
    for run in report["runs"]:
        ...
        lines.extend(
            [
                ...
                f"{run['test_benign_rows']} benign). Direct detection "
                f"`{primary['direct_detection_recall']:.5f}`, direct suspicious-unknown "
                f"`{primary['direct_suspicious_unknown_recall']:.5f}`, "
                f"detection-or-review `{primary['detection_or_review_recall']:.5f}`, "
                f"benign FPR `{primary['test_benign_fpr']:.5f}`.",
                "",
            ]
        )
    lines.extend(
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run cross-fitted environment-aware calibration challenger"
    )
    parser.add_argument(
        "--source",
        nargs=3,
    ...
    parser.add_argument("--experiment-id", default="DEV-CAL-001")
