...
from training.v2.causal_context import CAUSAL_SIDECAR_SCHEMA_VERSION
...
def validate_manifest(root: Path, manifest: dict[str, Any]) -> None:
    ...
    require(
        manifest["preparation_code_commit"] == PREPARATION_COMMIT,
        "context preparation commit changed",
    )
    require(
        manifest["sidecar_schema_version"] == CAUSAL_SIDECAR_SCHEMA_VERSION
...
def validate_local_sidecars(
    ...
    for report in manifest["scenarios"]:
        ...
        require(
            payload["schema_version"] == CAUSAL_SIDECAR_SCHEMA_VERSION
        ...
        for entry in entries:
            require(
                isinstance(entry, dict) and set(entry) == SIDECAR_ENTRY_KEYS,
...
def main() -> None:
    ...
    if args.sidecar_dir is not None and args.prepared_manifest is not None:
        validate_local_sidecars(args.sidecar_dir, args.prepared_manifest, manifest)
    print(
        "verified causal context preparation: 6 scenarios, 11771 history flows, "
        "7145 sealed rows; development only"
    )
